...
## Preview 2: automatic Office workspace

Setup detects desktop Office 15/16 executables (Office 2013 and the Office 2016+ / Microsoft 365 family), invokes Microsoft's VSTO trust installer and persists per-user startup registration (`LoadBehavior=3`, `file:` manifest plus `|vstolocal`). Native panes are created after Office startup, separately for each document window. The UI-thread timer waits for a complete window snapshot before removing stale panes; a busy Office call defers recovery. Closing a pane is respected until the user chooses Open Workspace or opens a new document window. Repeated Open calls are idempotent. Document identity supplements HWND identity to prevent a recycled window inheriting a previous document's workspace.
...
Excel undo now retains whether the original cell was a formula or a constant; formula-looking text is restored as literal text, and numeric constants retain their type. Regression tests cover each case.
