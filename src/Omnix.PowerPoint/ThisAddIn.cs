...
namespace Omnix.PowerPoint
{
    ...
    public sealed class ThisAddIn : AddInBase
    {
        ...
        protected override void FinishInitialization() { OnStartup();LocalData.Log("PowerPoint_STARTED"); }
        ...
        internal string RuntimeState => "OMNIX/4.0;host=PowerPoint;started="+(application!=null)+";panes="+windows.Count;
        internal void ShowWorkspace()
        {
            try {
                ...
                if(!windows.TryGetValue(handle,out pane)) {
                    ...
                    var view=new Workspace(application,"PowerPoint",Path.Combine(root,"gateway","Omnix.Gateway.exe"));
                ...
                LocalData.Log("PowerPoint_PANE_SHOWN");
            } catch(Exception e) {
        ...
        {
            foreach(var view in views)view.Dispose();views.Clear();windows.Clear();panes?.Dispose();base.OnShutdown();LocalData.Log("PowerPoint_STOPPED");
        }
    }
    [ComVisible(true)]
