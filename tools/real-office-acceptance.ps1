...
#
# What it tests for each installed host:
#   1. Office COM application can start.
#   2. OMNIX appears in Application.COMAddIns.
#   3. OMNIX Connect=True (or a controlled force-connect succeeds).
#   4. Office is closed cleanly.
#   5. Office is launched a second time and OMNIX is still present/connected.
#
...
function Get-OmnixRegistryState([string]$hostName, [string]$officeVersion) {
    ...
    return [pscustomobject]@{ Found=$false; VersionKey=$null; LoadBehavior=$null; Manifest=$null }
}
    ...
    try {
        ...
        if ($result.AddinFound -and -not $result.InitialConnect) {
            ...
            try {
                ...
                $result.Error = "Force-connect failed: $($_.Exception.GetType().FullName): $($_.Exception.Message)"
            }
        }
        ...
        $result.Pass = [bool]($result.Started -and $result.AddinFound -and $result.FinalConnect -and $result.Registry.Found -and ($result.Registry.LoadBehavior -eq 3))
    }
        ...
        if ($_.Exception.HResult -eq -2147221164) {
            ...
            $result.Pass = $true  # not applicable, not a product failure
        } else {
...
$overallPass = ($installedRows.Count -gt 0) -and (@($installedRows | Where-Object { -not $_.Pass }).Count -eq 0)
